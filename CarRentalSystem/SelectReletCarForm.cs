...
{
    public partial class SelectReletCarForm : Form
    {
        CarDal carDal = new CarDal();
        List<CarInfo> carInfos;
        private StoreDal storeDal = new StoreDal();
        public SelectReletCarForm()
        {
            InitializeComponent();
            RefreshList();
        }

        private void SelectReletCarForm_Load(object sender, EventArgs e)
        {
        ...
        private void btnSelectCar_Click(object sender, EventArgs e)
        {
            if (lvCarList.SelectedItems.Count > 0)
            {
                ...
                new ReletCarForm(carTemp).ShowDialog();
            }
            else
            {
                MessageBox.Show("请先选择要续租的车辆");
            }
        }
    }
}
